        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="px-2 py-0.5 bg-amber-400 text-zinc-950 text-[10px] font-bold uppercase tracking-[0.2em] rounded-sm italic">Migration Active</span>
              {/* <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest leading-none">Status: Ready for Deployment</span> */}
            </div>
          ...
          <div className="flex gap-4">
             <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-sm space-y-1">
                <p className="text-[10px] font-bold text-zinc-500 uppercase italic">File Extraction</p>
                <p className="text-2xl font-black text-white">{files.length}</p>
             </div>
             <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-sm space-y-1">
                <p className="text-[10px] font-bold text-zinc-500 uppercase italic">Env Readiness</p>
             ...
             {files.map((file: any, i: number) => (
               ...
               >
                 ...
                 <div className="space-y-1 overflow-hidden">
                    ...
                    <p className={`text-[10px] font-medium uppercase tracking-tighter italic truncate ${activeFile === i ? 'text-zinc-800' : 'text-zinc-400'}`}>{file.path}</p>
                 </div>
               </button>
             ))}
          </div>
          ...
          <div className="lg:col-span-8 bg-zinc-950 relative h-[600px] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
               ...
               <button 
                 ...
                 className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-bold uppercase hover:bg-zinc-800 hover:text-white transition-all"
               >
        ...
        <div className="space-y-6">
           ...
           <div className="bg-zinc-950 rounded-sm overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)]">
              <div className="bg-zinc-900 px-5 py-3 border-b border-zinc-800 flex justify-between items-center">
                 <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest italic">Industrial Startup Sequences</span>
              ...
              <div className="bg-zinc-50 border-2 border-zinc-950 rounded-sm p-6 space-y-4">
                 {data.server_setup?.steps?.map((step: string, i: number) => (
                   <div key={i} className="flex gap-4 group">
                      <div className="flex-shrink-0 w-5 h-5 bg-zinc-950 text-white flex items-center justify-center text-[10px] font-black rounded-[2px] italic shadow-[2px_2px_0px_0px_rgba(251,191,36,1)] group-hover:-translate-y-0.5 transition-transform">
                         {i + 1}
                      </div>
